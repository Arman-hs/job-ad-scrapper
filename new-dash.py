...
def add_bg_from_local(image_file):
    ...
    .stApp {{
        ...
        background-position:left 80% bottom 2%;
        background-size:55%;
    }}
    </style>
    """,
...
def charts(job_urls, matched_links, set_of_skills, user_inputs, base_url, location, search_query, skills_group, formatted_date):
    ...
    fig.update_layout(
        title=f"{base_url_without_hhtps} | {location} | {search_query} | {formatted_date}",
        xaxis_title="",
        yaxis_title="Count",
        width=600,
        height=450,
...
#            user_inputs = []
        ...
        with right_column:
             left_column, middle_column, right_column = st.columns((0.5,7,2))
             with middle_column:
                 st.markdown("##")
             ...
             with right_column:
                 ...
                 st.markdown("##")
                 st.markdown("##")
                 st.markdown(f'<p style="font-size:22px;">{"📆"} {formatted_date}</p>',
                            unsafe_allow_html=True)

             if "job_urls" in st.session_state:
                matched_links = st.session_state.matched_links
                
                charts(st.session_state.job_urls, matched_links, st.session_state.set_of_skills, user_inputs, base_url, location, search_query, skills_group, formatted_date)
